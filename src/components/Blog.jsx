...
export default function Blog({ post }) {
  ...
  return (
    <div className="py-5 flex items-start gap-5 md:items-center md:gap-4 border-b border-b-neutral-300">
      <div className="w-[2/5]">
        <div className="w-36 md:w-96 h-24 md:h-60 ">
          <Link href={`/${post._id}`}>
            <img
              src={post.image}
              alt={post.title}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <Link href={`/${post._id}`}>
          <h2 className="text-xl md:text-3xl font-medium font-serif w-full">
            {post.title}
          </h2>
        </Link>
        ...
        <p className="text-gray-800 text-sm w-4/5 hidden md:block">
          {post.excerpt.substring(0, 150)}...
        </p>
        <Link
          href={`/${post._id}`}
          className="text-blue-600 bg-blue-200 w-max px-2 py-1 rounded font-semibold text-sm md:flex items-center gap-1 hidden "
        >
